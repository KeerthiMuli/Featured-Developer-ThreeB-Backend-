...
public class LogOut extends SecurityContextLogoutHandler {
  ...
  /**
   * Create a new instance with a {@code ClientRegistrationRepository}, so that we can look up
   * information about the configured provider to call the Auth0 logout endpoint. Called by the
   * Spring framework.
   *
   * @param clientRegistrationRepository the {@code ClientRegistrationRepository} for this
   *     application.
   */
  @Autowired
  ...
  /**
   * Delegates to {@linkplain SecurityContextLogoutHandler} to log the user out of the application,
   * and then logs the user out of Auth0.
   *
  ...
  /**
   * Gets the Spring ClientRegistration, which we use to get the registered client ID and issuer for
   * building the {@code returnTo} query parameter when calling the Auth0 logout API.
   *
